...
Product *load_products(const char *path) {
    FILE *fp = fopen("C:\\Apache24\\data\\beverages.txt", "r");
...
void save_products(const char *path, Product *list) {
    FILE *fp = fopen("C:\\Apache24\\data\\beverages.txt", "w");
...
void print_html(Product *list) {
    ...
    puts("<html><head><title>Beverages</title>"
         "<meta charset='UTF-8'>"
         "<style>"
         ...
         "form{margin-top:8px;}"
         "</style>"
         "</head><body><h2>🥤 Beverage Store</h2>");
    ...
    for (Product *p = list; p; p = p->next) {
        ...
        if (p->stock > 0)
            printf("<form method='GET' action='/cgi-bin/beverages.cgi'>"
...
/* ---------- main ---------- */
int main(void) {
    const char *file = "C:\\Apache24\\data\\beverages.txt";
